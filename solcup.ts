...
const solcCached = (version:string) => Deno.stat(`${solcDir}/${version}`).then(_=>true).catch(_=>false)

export const getSolc = async (version:string) => {
    if (await solcCached(version)) return
